...
export function listenToPartnersByRole(role, callback) {
  ...
  // No orderBy here — avoids a composite index requirement; sort client-side instead
  const q = query(collection(db, 'partner_requests'), where('role', 'in', roles));
  return onSnapshot(q, snap => {
    const docs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    docs.sort((a, b) => {
      const ta = a.createdAt?.toDate ? a.createdAt.toDate() : new Date(a.createdAt || 0);
...
export async function getVendorOrders(vendorId) {
  ...
  const snap = await getDocs(query(collection(db, 'orders'), where('vendorId', '==', vendorId), limit(100)));
  const docs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  docs.sort((a, b) => {
    const ta = a.createdAt?.toDate ? a.createdAt.toDate() : new Date(a.createdAt || 0);
    const tb = b.createdAt?.toDate ? b.createdAt.toDate() : new Date(b.createdAt || 0);
...
// Backfill vendorId on old orders that contain this vendor's products but lack vendorId.
// Returns the count of orders patched.
export async function syncVendorOrders(vendorId) {
  ...
  snap.docs.forEach(d => {
    const data = d.data();
    if (data.vendorId) return; // already attributed
    const items = Array.isArray(data.items) ? data.items : [];
    if (items.some(i => productIds.has(i.productId))) {
      patches.push(updateDoc(doc(db, 'orders', d.id), { vendorId }));
    }
  });
